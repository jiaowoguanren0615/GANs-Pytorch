...
from models.dcgan import Generator, Discriminator, weights_init_normal
import torch



def get_args():
    parser = argparse.ArgumentParser(description='Train the DCGAN-model on images')
    parser.add_argument("--n_epochs", type=int, default=200, help="number of epochs of training")
    parser.add_argument("--model", type=str, default='DCGAN', help="name of model")
...
def main(args):
    ...
    generator = Generator(args.img_size, args.latent_dim, args.channels).to(device)
    discriminator = Discriminator(args.img_size, args.channels).to(device)

    generator.apply(weights_init_normal)
    discriminator.apply(weights_init_normal)

    # Optimizers
    optimizer_G = torch.optim.AdamW(generator.parameters(), lr=args.lr, betas=(args.b1, args.b2))
    optimizer_D = torch.optim.AdamW(discriminator.parameters(), lr=args.lr, betas=(args.b1, args.b2))
